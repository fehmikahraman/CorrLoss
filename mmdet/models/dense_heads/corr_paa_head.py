...
class CorrPAAHead(ATSSHead):
    ...
    def __init__(self,
        ...
        super(CorrPAAHead, self).__init__(*args, **kwargs)
        self.corr_w = corr_w
    ...
    def loss(self,
        ...
        if num_pos:
            ...
            # Fehmi CorrLoss
            labels_pos = labels[pos_inds_flatten]
            cls_scores_pos = cls_scores[pos_inds_flatten, labels_pos]
            cls_scores_pos = cls_scores_pos.sigmoid()
            if len(pos_inds_flatten) > 1:
                if self.corr_type == 'spearman':
                    loss_corr = self.spearmanr(cls_scores_pos, iou_target.clamp(min=EPS).detach())
                elif self.corr_type == 'concordance':
                    loss_corr = self.concordancer(cls_scores_pos, iou_target.clamp(min=EPS).detach())
                elif self.corr_type == 'pearson':
                    loss_corr = self.pearsonr(cls_scores_pos, iou_target.clamp(min=EPS).detach())
        ...
        return dict(
            loss_cls=losses_cls, loss_bbox=losses_bbox, loss_iou=losses_iou, loss_corr=losses_corr)
