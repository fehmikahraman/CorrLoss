...
class CorrGFLHead(AnchorHead):
    ...
    def __init__(self,
        ...
        self.reg_max = reg_max
        super(CorrGFLHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, labels, label_weights,
        ...
        if len(pos_inds) > 0:
            ...
            # estimate CorrLoss coefficient - Fehmi
            ious = bbox_overlaps(
                pos_decode_bbox_pred, pos_decode_bbox_targets, is_aligned=True).clamp(min=1e-6).detach()
        ...
        return loss_cls, loss_bbox, loss_dfl, weight_targets.sum(), loss_corr, fpn_level_active
    ...
    def loss(self,
        ...
        if self.use_loss_single:
            losses_cls, losses_bbox, losses_dfl,\
                avg_factor, losses_corr, fpn_levels_active = multi_apply(
                    self.loss_single,
                    anchor_list,
                    cls_scores,
                    bbox_preds,
        ...
        """else:
            ...
            all_bbox_preds = []
            all_strides = []
            for anc, labels, label_weights, cls_score, bbox_targets, bbox_pred, stride in zip(anchor_list,
                                                                                                  labels_list,
                                                                                                  label_weights_list,
                                                                                                  cls_scores,
                                                                                                  bbox_targets_list,
                ...
                all_bbox_preds.append(bbox_pred.permute(0, 2, 3, 1).reshape(-1, 4 * (self.reg_max+1)))
        ...
        """
        return dict(
            loss_cls=losses_cls, loss_bbox=losses_bbox, loss_dfl=losses_dfl, loss_corr=losses_corr)
