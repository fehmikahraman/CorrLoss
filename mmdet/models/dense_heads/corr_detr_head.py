...
class CorrDETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is CorrDETRHead):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is CorrDETRHead:
            convert_dict = {
    ...
    def loss(self,
        ...
        losses_cls, losses_bbox, losses_iou, losses_corr = multi_apply(
        ...
        loss_dict['loss_corr'] = losses_corr[-1]
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_iou_i, loss_corr_i in zip(losses_cls[:-1], losses_bbox[:-1], losses_iou[:-1],
                                                                    losses_corr[:-1]):
            loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_iou_i
            loss_dict[f'd{num_dec_layer}.loss_corr'] = loss_corr_i
            num_dec_layer += 1
        return loss_dict
    ...
    def loss_single(self,
        ...
        ious = bbox_overlaps(
            pos_bbox_pred, pos_bbox_targets, is_aligned=True).clamp(min=1e-6).detach()

        if len(pos_inds) > 1:
            labels_pos = labels[pos_inds]
            pos_score = cls_scores[pos_inds, :]
            pos_score_soft = pos_score.softmax(dim=1)
            cls_score_pos = pos_score_soft[range(len(labels_pos)), labels_pos]
            ...
            loss_corr = cls_scores.sum() * 0

        return loss_cls, loss_bbox, loss_iou, loss_corr
