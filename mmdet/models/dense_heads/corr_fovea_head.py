...
class CorrFoveaHead(AnchorFreeHead):
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            if num_pos > 1:
                corr_points = self.get_points(featmap_sizes, bbox_preds[0].dtype, bbox_preds[0].device)
                corr_flatten_bbox_targets = self.get_bbox_targets(flatten_bbox_targets, featmap_sizes, corr_points, num_imgs)
                corr_flatten_bbox_preds = self.get_bbox_targets(flatten_bbox_preds, featmap_sizes, corr_points, num_imgs)
                corr_pos_bbox_preds = corr_flatten_bbox_preds[pos_inds]
                corr_pos_bbox_targets = corr_flatten_bbox_targets[pos_inds]
                ious = bbox_overlaps(corr_pos_bbox_preds, corr_pos_bbox_targets,
                                     is_aligned=True).clamp(min=1e-6).detach()
                labels_pos = flatten_labels[pos_inds]
                cls_score_pos = flatten_cls_scores[pos_inds, labels_pos]
                ...
                loss_corr = loss_corr * self.corr_w
            else:
                loss_corr = torch.tensor(
                    0,
            ...
            loss_bbox = torch.tensor(
                ...
                dtype=flatten_bbox_preds.dtype,
                device=flatten_bbox_preds.device)
        return dict(loss_cls=loss_cls, loss_bbox=loss_bbox, loss_corr=loss_corr)
