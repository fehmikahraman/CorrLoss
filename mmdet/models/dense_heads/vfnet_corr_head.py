...
class VFNetCorrHead(ATSSHead, FCOSHead):
    ...
    def loss(self,
        ...
        if num_pos > 1:
            if self.use_corr == 1:
                # Apply to first stage
                pos_ios_ini = iou_targets_ini.clone().detach()
                cls_score_pos = flatten_cls_scores[pos_inds, pos_labels].sigmoid()
                if self.corr_type == 'spearman':
                    loss_corr = self.corr_w * self.spearmanr(cls_score_pos, pos_ios_ini)
                elif self.corr_type == 'concordance':
                    loss_corr = self.corr_w * self.concordancer(cls_score_pos, pos_ios_ini)
                elif self.corr_type == 'pearson':
                    loss_corr = self.corr_w * self.pearsonr(cls_score_pos, pos_ios_ini)
                if torch.isnan(loss_corr).any():
                    loss_corr = pos_ios_ini.sum() * 0
            elif self.use_corr == 2:
                # Apply to second stage
                cls_score_pos = flatten_cls_scores[pos_inds, pos_labels].sigmoid()
                if self.corr_type == 'spearman':
                    loss_corr = self.corr_w * self.spearmanr(cls_score_pos, pos_ious)
                elif self.corr_type == 'concordance':
                    loss_corr = self.corr_w * self.concordancer(cls_score_pos, pos_ious)
                elif self.corr_type == 'pearson':
                    loss_corr = self.corr_w * self.pearsonr(cls_score_pos, pos_ious)
                if torch.isnan(loss_corr).any():
                    loss_corr = pos_ious.sum() * 0
            elif self.use_corr == 3:            
                # Apply to both stages
                # Apply to first stage
                pos_ios_ini = iou_targets_ini.clone().detach()
                cls_score_pos = flatten_cls_scores[pos_inds, pos_labels].sigmoid()
                if self.corr_type == 'spearman':
                    loss_corr_ini = self.corr_w * self.spearmanr(cls_score_pos, pos_ios_ini)
                    # Apply to second stage
                    loss_corr_ref = self.corr_w * self.spearmanr(cls_score_pos, pos_ious)
                    # Average the loss
                    loss_corr = (loss_corr_ini + loss_corr_ref)/2
                elif self.corr_type == 'concordance':
                    loss_corr_ini = self.corr_w * self.concordancer(cls_score_pos, pos_ios_ini)
                    # Apply to second stage
                    loss_corr_ref = self.corr_w * self.concordancer(cls_score_pos, pos_ious)
                    # Average the loss
                    loss_corr = (loss_corr_ini + loss_corr_ref) / 2
                elif self.corr_type == 'pearson':
                    loss_corr_ini = self.corr_w * self.pearsonr(cls_score_pos, pos_ios_ini)
                    # Apply to second stage
                    loss_corr_ref = self.corr_w * self.pearsonr(cls_score_pos, pos_ious)
                    # Average the loss
                    loss_corr = (loss_corr_ini + loss_corr_ref) / 2
                if torch.isnan(loss_corr).any():
                    loss_corr = pos_ious.sum() * 0
            else:
                loss_corr = flatten_cls_scores.sum() * 0
        else:
            loss_corr = flatten_cls_scores.sum() * 0
