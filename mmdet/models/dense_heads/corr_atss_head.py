...
class CorrATSSHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(CorrATSSHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def loss_single(self, anchors, cls_score, bbox_pred, centerness, labels,
        ...
        if len(pos_inds) > 0:
            ...
            ious = bbox_overlaps(
                pos_decode_bbox_pred, pos_bbox_targets, is_aligned=True).clamp(min=1e-6).detach()
            labels_pos = labels[pos_inds]
            cls_score_pos = cls_score[pos_inds, labels_pos]
            cls_score_pos = cls_score_pos.sigmoid()
            if len(pos_inds) > self.corr_threshold:
                if self.corr_type == 'spearman':
                    loss_corr = self.spearmanr(cls_score_pos, ious, regularization_strength=self.regul_strength)
        ...
        return loss_cls, loss_bbox, loss_centerness, centerness_targets.sum(), loss_corr, fpn_level_active
    ...
    def loss(self,
        ...
        if self.use_loss_single:
            losses_cls, losses_bbox, loss_centerness,\
                bbox_avg_factor, losses_corr, fpn_levels_active = multi_apply(
                    self.loss_single,
                    anchor_list,
                    cls_scores,
                    bbox_preds,
            ...
            all_bbox_preds = []
            all_centerness = []
            for anc, labels, label_weights, cls_score, bbox_targets, bbox_pred, centerness in zip(anchor_list,
                                                                                                  labels_list,
                                                                                                  label_weights_list,
                                                                                                  cls_scores,
                                                                                                  bbox_targets_list,
                ...
                all_centerness.append(centerness.permute(0, 2, 3, 1).reshape(-1))
            ...
            centernesses = torch.cat(all_centerness)
            ...
            if len(pos_inds) > 0:
                ...
                pos_anchors = anchors[pos_inds]
                pos_centerness = centernesses[pos_inds]
                ...
                bbox_avg_factor = sum(centerness_targets)
                if bbox_avg_factor < 1e-12:
                    bbox_avg_factor = 1.0
                ...
                ious = bbox_overlaps(
                    pos_decode_bbox_pred, pos_bbox_targets, is_aligned=True).clamp(min=1e-6).detach()
                # giou = bbox_overlaps(pos_decode_bbox_pred, pos_decode_bbox_targets, mode='giou', is_aligned=True, eps=1e-6)
                ...
                if len(pos_inds) > self.corr_threshold:
                    ...
                    losses_corr = bbox_preds.sum() * 0
            else:
                losses_bbox = torch.cat(all_bbox_preds).sum() * 0
                losses_cls = torch.cat(all_cls_scores).sum() * 0
                loss_centerness = torch.cat(all_centerness).sum() * 0
                losses_corr = torch.cat(all_bbox_preds).sum() * 0

        return dict(
            loss_cls=losses_cls,
            loss_bbox=losses_bbox,
            loss_centerness=loss_centerness,
            loss_corr=losses_corr)
